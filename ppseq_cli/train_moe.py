...
from ppseq.criterions import CrossEntropyCriterionBase
    ...
    for batch_id, input_data in enumerate(dataloader):
        ...
        if amp is True:
            with paddle.amp.auto_cast():
                ...
                # loss = avg_cost
                balance_loss = model.get_balance_loss(alpha=1)
                loss = avg_cost + 1e-2 * balance_loss
        ...
        if (batch_id + 1) % log_steps == 0:
            ...
            logger.info(
                ...
                f"loss:{float(loss):.3f}, nll_loss:{float(nll_loss):.3f}, ppl:{float(ppl):.2f},moe_loss:{float(balance_loss):.2f} bsz:{bsz}, "
